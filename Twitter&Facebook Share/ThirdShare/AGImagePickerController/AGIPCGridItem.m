    ...
    @synchronized (self)
    {
        if (_selected != selected)
        {
            if (selected) {
                ...
                if (_imagePickerController.maximumNumberOfPhotosToBeSelected == numberOfSelectedGridItems) {
                    
                    [[[UIAlertView alloc] initWithTitle:[NSString stringWithFormat:@"你最多只能选择%@张照片", @(numberOfSelectedGridItems)] message:nil delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil] show];
                    return;
                }
