...
//       MATCHING_SYSTEM_VERSION_CODE_LATER_ADN_EARLIER(iOS7_AND_LATER, self.wantsFullScreenLayout = YES, nil) ;
//    }
...
- (void)changeSelectionInformation
{
    if (self.imagePickerController.shouldDisplaySelectionInformation ) {
        if (0 == [AGIPCGridItem numberOfSelections] ) {
            ...
            if (0 < maxNumber) {
                self.navigationController.navigationBar.topItem.prompt = [NSString stringWithFormat:@"(%ld/%ld)", [AGIPCGridItem numberOfSelections], maxNumber];
            } else {
                self.navigationController.navigationBar.topItem.prompt = [NSString stringWithFormat:@"(%ld/%ld)", [AGIPCGridItem numberOfSelections], self.assets.count];
            }
        }
    }
}
