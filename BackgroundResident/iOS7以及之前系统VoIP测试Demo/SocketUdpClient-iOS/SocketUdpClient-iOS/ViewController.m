...
{
    AsyncUdpSocket *_socketClient;
}

- (void)viewDidLoad {
    [super viewDidLoad];
    _socketClient = [[AsyncUdpSocket alloc] initWithDelegate:self];
    NSError *error;
    BOOL flag = [_socketClient bindToAddress:@"192.168.3.106" port:7777 error:&error];
    [_socketClient receiveWithTimeout:-1 tag:0];
    if (!flag || error) {
        NSLog(@"绑定失败 %@", error);
    }
...
- (IBAction)clickSendMsg:(id)sender {
    BOOL flag = [_socketClient sendData:[@"oooooooo" dataUsingEncoding:NSUTF8StringEncoding] toHost:@"192.168.3.106" port:8888 withTimeout:30 tag:0];
    if (!flag) {
        NSLog(@"发送失败");
    }
}
